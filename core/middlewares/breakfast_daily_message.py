...
async def send_daily_breakfast(bot, meal_type, percent, user_id):
    ...
    builder.button(text="Предложить рецепт", callback_data=f"покажи рецепт:{meal_type}:{round(bmr * percent * activity_level / 2.7,2)}")
    ...
    try:
        ...
        await bot.send_message(
                ...
                text=f"Привет, {user.name}! Время {res}! "
                     f"Желательно съесть {bmr * percent * activity_level / 2.7:.2f} ккал.",
